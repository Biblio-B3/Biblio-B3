...
app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const validatedBookStats = bookStats.map(stat => BookStatsSchema.parse(stat));
            res.status(200).json(validatedBookStats);

        } catch (error) {
            if (error instanceof AppError) return next(error);
            next(new Error("An error occurred while retrieving book statistics."));
        }
    }
);
...
app.get(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const validatedUserStats = userStats.map(stat => UserStatsSchema.parse(stat));
            res.status(200).json(validatedUserStats);

        } catch (error) {
            if (error instanceof AppError) return next(error);
            next(new Error("An error occurred while retrieving user statistics."));
        }
    }
);
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const reservationStats = await db
                ...
                .groupBy(sql`date_trunc('${sql.raw(period)}', ${reservation.reservation_date})`)
                .orderBy(sql`date_trunc('${sql.raw(period)}', ${reservation.reservation_date})`);
            ...
            const readStats = await db
                ...
                .groupBy(sql`date_trunc('${sql.raw(period)}', ${historical.date_read})`)
                .orderBy(sql`date_trunc('${sql.raw(period)}', ${historical.date_read})`);
            ...
            const userStats = await db
                ...
                .groupBy(sql`date_trunc('${sql.raw(period)}', ${users.created_at})`)
                .orderBy(sql`date_trunc('${sql.raw(period)}', ${users.created_at})`);
            ...
            const overdueStats = await db
                ...
                .groupBy(sql`date_trunc('${sql.raw(period)}', ${reservation.final_date})`)
                .orderBy(sql`date_trunc('${sql.raw(period)}', ${reservation.final_date})`);
            ...
            reservationStats.forEach(stat => {
                if (statsMap.has(stat.period)) {
                    statsMap.get(stat.period)!.reservations_count = Number(stat.reservations_count);
                }
            });

            readStats.forEach(stat => {
                if (statsMap.has(stat.period)) {
                    statsMap.get(stat.period)!.reads_count = Number(stat.reads_count);
                }
            });

            userStats.forEach(stat => {
                if (statsMap.has(stat.period)) {
                    statsMap.get(stat.period)!.new_users_count = Number(stat.new_users_count);
                }
            });

            overdueStats.forEach(stat => {
                if (statsMap.has(stat.period)) {
                    statsMap.get(stat.period)!.overdue_count = Number(stat.overdue_count);
                }
            });

            // Convertir en array et trier
            const temporalStatsArray = Array.from(statsMap.values()).sort((a, b) => a.period.localeCompare(b.period));

            const validatedTemporalStats = temporalStatsArray.map(stat => TemporalStatsSchema.parse(stat));
            res.status(200).json(validatedTemporalStats);

        } catch (error) {
            if (error instanceof AppError) return next(error);
            next(new Error("An error occurred while retrieving temporal statistics."));
        }
    }
);
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const occupationByBook = await db
                ...
                .where(eq(books.is_removed, false))
                .groupBy(books.id, books.title, books.author)
                .orderBy(desc(sql`CASE WHEN COUNT(DISTINCT ${copy.id}) > 0 THEN ROUND((COUNT(DISTINCT CASE WHEN ${reservation.final_date} >= ${now} THEN ${reservation.copy_id} END)::numeric / COUNT(DISTINCT ${copy.id})::numeric) * 100, 2) ELSE 0 END`));
            ...
            if (error instanceof AppError) return next(error);
            next(new Error("An error occurred while retrieving occupation statistics."));
        }
    }
);
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const overdueReservations = await db
                ...
                .orderBy(desc(sql`EXTRACT(DAY FROM (${now} - ${reservation.final_date}))`));
            ...
            if (error instanceof AppError) return next(error);
            next(new Error("An error occurred while retrieving overdue statistics."));
        }
    }
);
